...
const kSources = Symbol('kSources');
const kLazyLoad = Symbol('kGetHandle');
...
function getSource(source, endings) {
  if (isBlob(source))
    return [source.size, source[kLazyLoad]().handle];
...
class Blob {
  ...
  [kLazyLoad]() {
    if (this[kSources]) {
      this[kHandle] = _createBlob(this[kSources], this[kLength]);
      this[kType] = RegExpPrototypeExec(disallowedTypeCharacters, this[kType]) !== null ?
        '' : StringPrototypeToLowerCase(this[kType]);
      this[kSources] = null;
    }

    return { handle: this[kHandle], type: this[kType] };
  }
  ...
  [kClone]() {
    ...
    const { handle, type } = this[kLazyLoad]();
  ...
  slice(start = 0, end = this[kLength], contentType = '') {
    ...
    return createBlob(
      this[kLazyLoad]().handle.slice(start, start + span),
      span,
      contentType);
  }
  ...
  arrayBuffer() {
    ...
    const reader = this[kLazyLoad]().handle.getReader();
    const buffers = [];
    const readNext = () => {
