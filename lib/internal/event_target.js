...
const { validateAbortSignal, validateDictionary, validateString, validateInternalField } = require('internal/validators');
...
class EventTarget {
  ...
  // REMOVE THIS; PRIVATE PROPS DO NOT WORK CROSS-REALM
  /** @type {Map<string, EventListenerStruct[]>} */
  kTestIndices = new SafeMap()
   ...
   * @typedef {Object} EventListenerStruct
   * @property {string} type
   * @property {EventTargetCallback|null} callback
   * @property {boolean} capture
   * @property {boolean} passive
   * @property {boolean} [once=false]
   * @property {import('./abort_controller').AbortSignal|null} signal
  ...
  /**
   * @param {string} type
   * @param {EventTargetCallback|EventListener} callback
   * @param {{
   *   capture?: boolean,
   *   once?: boolean,
   *   passive?: boolean,
   *   signal?: import('./abort_controller').AbortSignal
   * }} [options]
   */
  addEventListener(type, callback, options = kEmptyObject) {
    ...
    if (!validateEventListener(callback)) {
      // The DOM silently allows passing undefined as a second argument
      // No error code for this since it is a Warning
      // eslint-disable-next-line no-restricted-syntax
      const w = new Error(`addEventListener called with ${callback}` +
                          ' which has no effect.');
   ...
   * }} [options]
   */
  removeEventListener(type, callback, options = kEmptyObject) {
    if (!isEventTarget(this))
      throw new ERR_INVALID_THIS('EventTarget');
    if (arguments.length < 2)
      throw new ERR_MISSING_ARGS('type', 'listener');
    ...
    const { capture } = flattenMoreOptions(options)
    type = webidl.converters.DOMString(type);

    const eventList = this.kTestIndices.get(type)

    if (!eventList) {
      return
    }

    const eventListener = ArrayPrototypeFind(eventList, (value) => {
      return value.type === type && value.capture === capture && value.callback === callback
    })
    ...
    const index = ArrayPrototypeIndexOf(eventList, eventListener)

    eventListener.removed = true

    ArrayPrototypeSplice(eventList, index, 1)

    if (!eventList.length) {
      this.kTestIndices.delete(type)
    }
  }
 ...
 */
function flattenMoreOptions (options) {
  ...
  const signal = options.signal // only hit getter once
  if (signal != null)
    validateAbortSignal(signal, 'signal')

  validatedOptions.signal = signal
 ...
 * @param {EventListenerStruct} listener
 */
function addAnEventListener (eventTarget, listener) {
  ...
  const event = eventTarget.kTestIndices.get(listener.type)

  if (!event) {
    eventTarget.kTestIndices.set(listener.type, [listener])
  } else if (
    !ArrayPrototypeSome(event, (value) => {
      return value.callback === listener.callback && value.capture === listener.capture
    })
  ) {
    ArrayPrototypePush(event, listener)
    eventTarget.kTestIndices.set(listener.type, event)
  }

  if (listener.signal) {
    listener.signal.addEventListener('abort', () => {
      eventTarget.removeEventListener(listener.type, listener.callback, listener)
    }, { once: true, [kWeakHandler]: eventTarget })
  }

  // TODO: call eventTarget[kNewListener]
}
